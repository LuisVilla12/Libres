...
//print_r($TotalCargo);
//echo $tipoPoliza;
$t= new reg_cuentas($Num_Poliza, $idEjercicio);
$x=$db->select($t->selectDatosPoliza($Num_Poliza, $idEjercicio));
//echo json_encode($x);
//print_r($x);

if($x){ 
$Asientos=dibujarCampPolizas($x);
  //exit();
}
...
$cuentas_Codigo_Cuenta=$x[0]['cuentas_Codigo_Cuenta'];
...
<htmlpageheader name="myHeader">
           ...
           <div id="header-content">
             ...
             <br>
             <h3 align="center">Póliza de '.$tipoPoliza.'</h3>
           </div>
        </htmlpageheader>
...
<table class="table" width="100%">
            ...
            <td align="right" ><strong>$ '.number_format($Cargo, 2, '.', ',').'</strong></td>
            <td align="right" style="color: white; background-color: #1C2D66;"><strong>Abono:</strong></td>
            <td align="right" style="width:20%" ><strong>$ '.number_format($Abono, 2, '.', ',').'</strong>
            </td>
          </tr>
                </tfoot>
             </table>
...
$mpdf->Output('Póliza '.$Num_Poliza.'.pdf','i');

function dibujarCampPolizas($x){
  $docPoliza="";
  foreach ($x as $poliza)
    {  
      setlocale(LC_TIME, 'spanish');
      $docPoliza .= '<tr> 
       <td class="noborde centrar tam12 style1">'.$poliza['Num_Poliza'].'</td>
       <td class="noborde centrar tam12 style1">'.utf8_encode(strftime("%A, %d de %B del %Y", strtotime($poliza['Fecha_Factura']))).'</td>
       <td class="noborde centrar tam12 style1">'.utf8_encode(strftime("%A, %d de %B del %Y", strtotime($poliza['Fecha_Reg']))).'</td>
       <td class="noborde centrar tam12 style1">'.$poliza['cuentas_Codigo_Cuenta'].'</td>
       <td class="noborde centrar tam12 style1">'.$poliza['Nombre_Cuenta'].'</td>
       <td class="noborde centrar tam12 style1">'.$poliza['Concepto'].'</td>
       <td class="noborde centrar tam12 style1">$ '.number_format($poliza['Monto'], 2, '.', ',').'</td>
       <td class="noborde centrar tam12 style1">'.$poliza['Codigo_Naturaleza'].'</td></tr>';
     }
  return $docPoliza;
}
